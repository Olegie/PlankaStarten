...
Console executables generated by PlankaStarten can also run without command
line arguments. In that mode they choose an entry procedure, ask for `V`
arguments through stdin, print a `Result` block, and wait before closing.
...
plankastarten_cli app <file.plk> [procedure]
...
`app` is the interactive runner. It loads the `.plk` file, chooses `start` when
available, prompts for procedure inputs and prints the results.
